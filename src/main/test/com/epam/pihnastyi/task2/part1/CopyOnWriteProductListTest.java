...
public class CopyOnWriteProductListTest {
    ...
    CopyOnWriteProductList<Product> list;
    ...
    @Before
    public void setUp() {
        list = new CopyOnWriteProductList<>(3);
        initList = new CopyOnWriteProductList<>(3);
        initListWithNull = new CopyOnWriteProductList<>(3);
    ...
    @Test(expected = NoSuchElementException.class)
    public void runThroughCollectionWithNextWrongInPredicateIteratorShouldNoSuchElementException() {
        fillInList(list);
    ...
    @Test
    public void iteratorShouldGoingOnOldArray() {
        fillInList(list);
        Iterator iterator = list.iterator();
        Product product4 = new Product("product3", "color3", 3);
        list.add(product4);
        int index = 0;
        while (iterator.hasNext()) {
            iterator.next();
    ...
    @Test
    public void removeSeveralTimeThisElementInIteratorShouldFalse() {
        fillInList(list);
        iterator = list.iterator();
        Product deletedProduct = iterator.next();
        list.remove(deletedProduct);
        assertFalse(list.remove(deletedProduct));
    }

    @Test
    public void removeSeveralTimeThisElementInPredicateFalse() {
        fillInList(list);
        Predicate<Product> predicate = product -> product.getName().equals("product2");
        iterator = list.iterator(predicate);
        Product deletedProduct = iterator.next();
    ...
    @Test(expected = NoSuchElementException.class)
    public void runThroughCollectionMoreThanPredicateCollectionShouldNoSuchElementException() {
        fillInList(list);
        Predicate<Product> predicate = product -> product.getName().equals("product2");
        iterator = list.iterator(predicate);
        while (iterator.hasNext()) {
