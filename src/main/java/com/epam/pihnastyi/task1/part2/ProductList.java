...
public class ProductList<E extends Product> implements List<E> {
    ...
    private class ProductIterator<E extends Product> implements Iterator<E> {
        ...
        @Override
        public boolean hasNext() {
            for (int i = index; i < productsArray.length; i++) {
                if (predicate.test((E) productsArray[i])) {
                    return true;
                }
            }
            return false;
